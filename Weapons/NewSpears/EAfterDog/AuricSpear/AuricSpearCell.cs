...
{
    internal class AuricSpearCell : ModProjectile, ILocalizedModType
    {
        public new string LocalizationCategory => "Projectiles.NewSpears.EAfterDog";
        ...
        private bool hasLockedOnTarget = false; // 是否已经搜索过目标
        ...
        public override void AI()
        {
            ...
            else if (!hasLockedOnTarget) // **第 11 帧时搜索最近的敌人**
            {
                NPC target = Projectile.Center.ClosestNPCAt(2400);
                if (target != null)
                {
                    Vector2 direction = (target.Center - Projectile.Center).SafeNormalize(Vector2.Zero);
                    Projectile.velocity = direction * 18f; // **冲刺速度**
                    hasLockedOnTarget = true; // **标记已搜索，不再重复调整**
                }
            }
